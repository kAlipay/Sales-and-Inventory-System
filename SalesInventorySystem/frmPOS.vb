...
Public Class frmPOS
    ...
    Private Function isAvailableQuantity() As Boolean
        Try
            sqL = "SELECT StocksOnHand FROM TblProduct WHERE ItemNo = " & Val(txtSearch.Text) & ""
            ConnDB()
    ...
    Private Sub GetItemInfo()
        Try
            sqL = "SELECT ProductID, ProductName, UnitPrice, itemNo FROM TblProduct Where ItemNo = " & Val(txtSearch.Text) & ""
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
    ...
    Private Sub UpdateDecreaseQuantity()
        Try
            sqL = "Update TblProduct SET StocksOnHand = stocksOnHand - '" & itemQuantity & "' WHERE ItemNo = " & itemNum & ""
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            cmd.ExecuteNonQuery()
    ...
    Private Sub GetProductIDToDelete()
        Try
            sqL = "SELECT ItemNo FROM TblProduct Where ProductID = '" & dgw.CurrentRow.Cells(1).Value & "'"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            If dr.Read = True Then
    ...
    Private Sub UpdateIncreaseQuantity()
        Try
            sqL = "Update TblProduct SET StocksOnHand = stocksOnHand + '" & dgw.CurrentRow.Cells(0).Value & "' WHERE ItemNo = " & getProdtoDelete & ""
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            cmd.ExecuteNonQuery()
    ...
    Public Sub AddPOS()
        ...
        Try
            sqL = "INSERT INTO POS(InvoiceNo, POSDate, POSTime, NonVatAmount, VatAmount, TotalAmount, StaffID, CustomerNo) VALUES('" & lblInvoiceNo.Text & "', '" & Format(Date.Now, "Short Date") & "', '" & Format(Date.Now, "Long Time") & "', '" & nonVatAmount & "', '" & vatAmount & "', '" & lblTotalCost.Text & "', '" & frmmain.statuser.Text & "'," & Val(lblCustomerNo.Text) & ")"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
    ...
    Private Sub btnPayment_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPayment.Click
        frmPayment.Show()
        ' lblCustomerName.Text = frmPrintReceipt.lblCustomerName.Text
    End Sub

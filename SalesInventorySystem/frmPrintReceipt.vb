...
Public Class frmPrintReceipt
    ...
    Private Sub LoadCustomer()
        Try
            sqL = "SELECT CustName FROM TblCustomers As C, POS as P WHERE C.CustomerNo = P.CustomerNo AND P.InvoiceNo = '" & frmPOS.lblInvoiceNo.Text & "'"
            ConnDB()
    ...
    Private Sub getEmployee()
        Try
            sqL = "SELECT S.UserID, FullName FROM TblUser as S, POS as P WHERE S.UserID = P.UserID AND InvoiceNo = '" & frmPOS.lblInvoiceNo.Text & "'"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
    ...
    Private Sub LoadReceipt()
        ...
        Dim Y As Integer = 0
        Try
            sqL = "SELECT ProductName, itemprice, [StockOnHand], POSDATE, (itemPrice * [Quantity]) as TotalPrice FROM [TblProduct] as I, POSDetail as PD, POS as P WHERE I.ItemNo = PD.itemNo And PD.InvoiceNO = P.InvoiceNo AND P.InvoiceNo = '" & frmPOS.lblInvoiceNo.Text & "' ORDER By ProductName"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
            ...
            Do While dr.Read = True
                dgw.Rows.Add(dr("Quantity"), dr("ProductName"), dr("Itemprice"), dr("TotalPrice"))

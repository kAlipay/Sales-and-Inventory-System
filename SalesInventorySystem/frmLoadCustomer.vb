...
Public Class frmLoadCustomer
    ...
    Private Sub LoadCustomer()
        Try
            sqL = "SELECT CustomerNo, CustName, Address FROM TblCustomers Order By CustName"
            ConnDB()
    ...
    Private Sub SearchCustomer()
        Try
            sqL = "SELECT CustomerNo, CustName, Address FROM TblCustomers WHERE CustName LIKE '" & TextBox1.Text & "%' Order By CustName"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()

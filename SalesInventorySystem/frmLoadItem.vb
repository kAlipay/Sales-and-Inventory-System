...
Public Class frmLoadItem
    ...
    Private Sub LoadItem()
        Try
            sqL = "SELECT ItemNo, ProductID, ProductName, StocksOnHand FROM TblProduct Order By ProductName"
            ConnDB()
    ...
    Private Sub Search()
        Try
            sqL = "SELECT ItemNo, ProductID, ProductName, StocksOnHand FROM TblProduct WHERE ProductName LIKE '" & TextBox1.Text & "%' Order By ProductName"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
